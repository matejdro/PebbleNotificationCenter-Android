...
public class PebbleTalkerService extends Service {
  ...
  private SparseArray<PendingNotification> sentNotifications =
      new SparseArray<PendingNotification>();
  ...
  public int onStartCommand(Intent intent, int flags, int startId) {
    ...
    if (intent != null && intent.hasExtra("id")) {
      ...
      notifyInternal(id, pkg, tag, title, subtitle, text, dismissable, noHistory,
          isListNotification);
    } else
      appOpened();
  ...
  private void send(PendingNotification notification) {
    ...
    configBytes[0] = Byte.parseByte(settings.getString("textSize", "0")); // Text
    // size
  ...
  private void dismissOnPebbleInternal(Integer androidId, String pkg, String tag, boolean dontClose) {
    ...
    for (int i = 0; i < sentNotifications.size(); i++) {
      ...
      if (!notification.isListNotification && notification.androidID != null
          && notification.androidID.intValue() == androidId.intValue() && notification.pkg != null
          && notification.pkg.equals(pkg)
          && (notification.tag == null || notification.tag.equals(tag))) {
  ...
  private void notifyInternal(Integer androidID, String pkg, String tag, String title,
      String subtitle, String text, boolean dismissable, boolean noHistory,
      boolean isListNotification) {
    ...
    if (!isListNotification) {
      if (notification.androidID != null) {
        ...
        for (int i = 0; i < sentNotifications.size(); i++) {
          ...
          if (!notification.isListNotification && notification.androidID == comparing.androidID
              && comparing.text.equals(notification.text)
              && comparing.title.equals(notification.title)
              && comparing.subtitle.equals(notification.subtitle)) {
            return;
          }
        }
      ...
      if (settings.getBoolean("enableQuietTime", false)) {
        ...
        if ((endTime > startTime && curTime <= endTime && curTime >= startTime)
            || (endTime < startTime && (curTime <= endTime || curTime >= startTime))) {
          return;
        }
      }

      if (settings.getBoolean("noNotificationsNoPebble", false)
          && !PebbleKit.isWatchConnected(this))
        return;
    }
  ...
  private void closeApp() {
    ...
    if (settings.getBoolean(PebbleNotificationCenter.CLOSE_TO_LAST_CLOSED, true)
        && previousUUID != null)
  ...
  private void dismissRequested(PebbleDictionary data) {
    ...
    PendingNotification notification = sentNotifications.get(id);
    if (notification != null) {
      JellybeanNotificationListener.dismissNotification(notification.pkg, notification.tag,
  ...
  private void sendConfig(boolean notificationWaiting) {
    ...
    configBytes[0] =
        (byte) Integer.parseInt(settings.getString(PebbleNotificationCenter.FONT_TITLE, "6"));
    configBytes[1] =
        (byte) Integer.parseInt(settings.getString(PebbleNotificationCenter.FONT_SUBTITLE, "5"));
    configBytes[2] =
        (byte) Integer.parseInt(settings.getString(PebbleNotificationCenter.FONT_BODY, "4"));
    configBytes[3] = (byte) (timeout >>> 0x08);
    configBytes[4] = (byte) timeout;
    configBytes[5] =
        (byte) Math.min(200,
            Integer.parseInt(settings.getString("vibratePeriodicallyPeriod", "20")));
    configBytes[6] =
        (byte) Integer.parseInt(settings.getString(PebbleNotificationCenter.VIBRATION_MODE, "4"));;

    byte flags = 0;
    flags |= (byte) (settings.getBoolean("autoSwitch", false) ? 0x01 : 0);
    flags |=
        (byte) (settings.getBoolean(PebbleNotificationCenter.CLOSE_TO_LAST_CLOSED, false) ? 0x02
            : 0);
    flags |= (byte) (NotificationHandler.isNotificationListenerSupported() ? 0x04 : 0);
    flags |= (byte) (notificationWaiting ? 0x08 : 0);

    boolean backlight = settings.getBoolean(PebbleNotificationCenter.LIGHT_SCREEN_ON_SUNSET_NOTIFICATION, false);
    if (backlight) {
      locationLookup.lookup();
      double latitude = settings.getFloat(PebbleNotificationCenter.PreferencesLatitude, 0);
      double longitude = settings.getFloat(PebbleNotificationCenter.PreferencesLongitude, 0);
      Location location = new Location(latitude, longitude);
      SunriseSunsetCalculator sunriseSunsetCalculator =
          new SunriseSunsetCalculator(location, TimeZone.getDefault());
      boolean betweenSunsetSunrise =
          TimeUtil.isBetweenTimes(Calendar.getInstance(),
      ...
      flags |=
          (byte) (settings.getBoolean(PebbleNotificationCenter.LIGHT_SCREEN_ON_NOTIFICATION, false) ? 0x10
              : 0);
    }
    flags |=
        (byte) (settings.getBoolean(PebbleNotificationCenter.DONT_VIBRATE_WHEN_CHARGING, true) ? 0x20
            : 0);
    ...
    configBytes[10] =
        (byte) Integer.parseInt(settings.getString(PebbleNotificationCenter.SHAKE_ACTION, "1"));
  ...
  private void packetInternal(int id, PebbleDictionary data) {
    switch (id) {
      ...
      case 4:
        if (listHandler != null)
          listHandler.gotRequest(data);
        break;
      case 5:
        if (listHandler != null)
          listHandler.entrySelected(data);
        break;
      case 6:
        menuPicked(data);
        break;
      ...
      case 8:
        if (listHandler != null)
          listHandler.sendRelativeNotification(data);
        break;
      case 9:
  ...
  public static void notify(Context context, String title, String subtitle, String text,
      boolean noHistory, boolean isListNotification) {
    notify(context, null, null, null, title, subtitle, text, false, noHistory, isListNotification);
  }

  public static void notify(Context context, Integer id, String pkg, String tag, String title,
      String subtitle, String text, boolean dismissable) {
  ...
  public static void notify(Context context, Integer id, String pkg, String tag, String title,
    ...
    if (service == null) {
      ...
      service.notifyInternal(id, pkg, tag, title, subtitle, text, dismissable, noHistory,
          isListNotification);
    }
  }
  ...
  public static void gotPacket(final Context context, final int packetId,
